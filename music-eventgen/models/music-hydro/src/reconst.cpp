...
ReconstCell Reconst::ReconstIt_shell(double tau, const TJbVec &tauq_vec,
    ...
    int flag = ReconstIt_velocity_Newton(grid_p1, tau, q_vec, grid_pt, flag2);
...
int Reconst::ReconstIt_velocity_Newton(ReconstCell &grid_p, double tau,
                                       const TJbVec &q,
                                       const Cell_small &grid_pt, const int flag) {
    ...
    int v_status = solve_v_Hybrid(v_guess, T00, M, J0, proper_tau, v_solution);
    if (v_status == 0) {
        return(-1);
    }
    ...
    if (v_solution > v_critical) {  // for large velocity, solve u0
        double u0_guess = u[0]; //1./sqrt(1. - v_solution*v_solution);
        double u0_solution = u0_guess;

        //int u0_status = solve_u0_Newton(u0_guess, T00, K00, M, J0, proper_tau, u0_solution);
	int u0_status = solve_u0_Hybrid(u0_guess, T00, K00, M, J0, proper_tau, u0_solution);
        if (u0_status == 0) {
            return(-1);
        }
        u[0] = u0_solution;
    ...
    if (check_u0_var > 100) {
        if (grid_pt.epsilon > 5e-3 && echo_level > 2) {
    ...
    grid_p.proper_tau = proper_tau + (1./DATA.rk_order)*DATA.delta_tau/grid_p.u[0];
...
int Reconst::solve_velocity_Newton(const double v_guess, const double T00,
    ...
    if (v_status == 0 && echo_level > 5) {
       music_message.warning(
               "Reconst velocity v Newton:: can not find solution!");
...
int Reconst::solve_v_Hybrid(const double v_guess, const double T00,
    ...
    reconst_velocity_fdf(v_l, T00, M, J0, proper_tau, fv_l, dfdv_l);
    reconst_velocity_fdf(v_h, T00, M, J0, proper_tau, fv_h, dfdv_h);
    ...
    double fv, dfdv;
    reconst_velocity_fdf(v_root, T00, M, J0, proper_tau, fv, dfdv);
    double abs_error_v = 10.0;
    double rel_error_v = 10.0;
    int iter_v = 0;
    do {
...
int Reconst::solve_u0_Newton(const double u0_guess, const double T00,
    ...
    if (u0_status == 0 && echo_level > 5) {
        music_message.warning(
                "Reconst velocity u0 Newton:: can not find solution!");
        music_message.warning(
                "output the results at the last iteration:");
        music_message.warning("iter  [lower, upper]  root  err(est)");
...
int Reconst::solve_u0_Hybrid(const double u0_guess, const double T00,
    ...
    reconst_u0_fdf(u0_l, T00, K00, M, J0, proper_tau, fu0_l, dfdu0_l);
    reconst_u0_fdf(u0_h, T00, K00, M, J0, proper_tau, fu0_h, dfdu0_h);
    ...
    if (fu0_l*fu0_h > 0.) {
        ...
        music_message << "u0_guess = " << u0_guess << ", T00 = " << T00
                      << ", M = " << M << ", J0 = " << J0 << ", u0_l = "
		      << u0_l << ", u0_h = " << u0_h << ", fu0_l = " << fu0_l << ", fu0_h = " << fu0_h
	              << ", e_l = " << T00-sqrt(1-1/(u0_l*u0_l))*M << ", e_h = " << T00-sqrt(1-1/(u0_h*u0_h))*M
	              << ", P_l = " << eos.get_pressure(T00-sqrt(1-1/(u0_l*u0_l)),0,proper_tau)
	              << ", P_h = " << eos.get_pressure(T00-sqrt(1-1/(u0_h*u0_h)),0,proper_tau);
        music_message.flush("error");
        return(u0_status);
    }
    ...
    double fu0, dfdu0;
    reconst_u0_fdf(u0_root, T00, K00, M, J0, proper_tau, fu0, dfdu0);
    double abs_error_u0 = 10.0;
    double rel_error_u0 = 10.0;
    int iter_u0 = 0;
    do {
