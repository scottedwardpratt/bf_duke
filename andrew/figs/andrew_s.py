...
mydata = np.loadtxt('eos_vs_epsilon.txt',skiprows=0,unpack=True)
epsilon=mydata[0]
T=1000*mydata[1]
s=mydata[2]
chiuu=mydata[3]
...
plt.plot(T,s,linestyle='-',linewidth=4,color='r',marker=None)
plt.plot(T_hadron,s_hadron,linestyle='-',linewidth=2,color='g',marker=None)
plt.plot(T_lattice,s_lattice,linestyle='-',linewidth=2,color='b',marker=None)
...
ax.set_yticks(np.arange(0,161,40), minor=False)
ax.set_yticklabels(np.arange(0,161,40), minor=False, family='serif')
ax.set_yticks(np.arange(0,161,10), minor=True)
...
plt.ylabel('$s$ (fm$^{-3}$)',fontsize=18)
#plt.title('MathText Number $\sum_{n=1}^\infty({-e^{i\pi}}/{2^n})$!',
#fontsize=12, color='gray')
#plt.subplots_adjust(top=0.85)
plt.savefig('s_andrew.pdf',format='pdf')
os.system('open -a Preview s_andrew.pdf')
#plt.show()
quit()
